    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (email, pwd_hash, display_name or email.split('@')[0], USER_EXPIRE_MINUTES),
            )
            conn.commit()
            return {
                'id': cur.lastrowid,
...
def login_user(email: str, password: str) -> dict | None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 检查是否过期（非管理员，expire_at 是 MySQL TIMESTAMP = UTC）
            if not row[4] and row[6] and row[6] < datetime.datetime.utcnow():
...
def get_user_by_id(user_id: int) -> dict | None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 非管理员检查过期（expire_at 是 MySQL TIMESTAMP = UTC）
            if not row[3] and row[6] and row[6] < datetime.datetime.utcnow():
                cur.execute('DELETE FROM users WHERE id = %s', (row[0],))
                conn.commit()
                return None
